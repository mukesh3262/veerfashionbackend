<?php

declare(strict_types=1);

namespace App\Http\Requests\Api\V1;

use App\Rules\Api\EnsureEmailIsNotVerifiedRule;
use App\Rules\Api\EnsureMobileIsNotVerifiedRule;
use App\Rules\Api\VerifyOtpRule;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class VerifyOTPRequest extends FormRequest
{
    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, \Illuminate\Contracts\Validation\ValidationRule|array<mixed>|string>
     */
    public function rules(): array
    {
        $user = request()->user();

        return [
            'verification_for' => ['required', 'in:email,mobile'],

            'email' => [
                'required_if:verification_for,email',
                'email',
                Rule::unique('users', 'email')->ignore($user->id),
                new EnsureEmailIsNotVerifiedRule,
            ],

            'isd_code' => ['required_if:verification_for,mobile'],

            'mobile' => [
                'required_if:verification_for,mobile',
                Rule::unique('users', 'mobile')->ignore($user->id),
                new EnsureMobileIsNotVerifiedRule,
            ],

            'otp' => ['required', 'string', new VerifyOtpRule],
        ];
    }

    protected function prepareForValidation(): void
    {
        $this->merge(['email' => mb_strtolower($this?->email ?? '')]);
    }
}
