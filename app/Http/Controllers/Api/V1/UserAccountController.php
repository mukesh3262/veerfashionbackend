...
class UserAccountController extends Controller
{
        ...
        try {
            ...
            $user = tap($request->user(), function (User $user) use ($request) {
                ...
                if ($request->hasFile('profile_photo')) {
                    ...
                    $profilePhotoName = now()->timestamp . '.' . $profilePhoto->getClientOriginalExtension();
                    Storage::disk(config('filesystems.default'))->putFileAs(config('constants.s3.customer_profile'), $profilePhoto, $profilePhotoName);
    ...
    public function deleteUserAccount(Request $request): JsonResponse
    {
        try {
            ...
            return FacadesResponse::success(
                message: __('basecode/api.deleted', ['entity' => 'Your account']),
                statusCode: SymfonyResponse::HTTP_OK
            );
        } catch (Throwable $th) {
            return $this->exceptionResponse($th);
